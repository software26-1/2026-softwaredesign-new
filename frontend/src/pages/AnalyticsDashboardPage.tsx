...
function KpiCard({ label, value, sub, color }: { label: string; value: string; sub?: string; color: string }) {
  return (
    <div style={{ background: '#fff', borderRadius: '10px', padding: '18px 20px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', borderTop: `3px solid ${color}` }}>
      <p style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 600, marginBottom: '8px', letterSpacing: '0.02em' }}>{label}</p>
...
function StudentAnalyticsView({ studentId }: { studentId: number }) {
  ...
  return (
    <div>
      ...
      `}</style>
      <div className="analytics-kpi-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
        <KpiCard label="전체 평균" value={summary?.overallAvgScore != null ? `${summary.overallAvgScore.toFixed(1)}점` : '-'} color="#1e5a99" />
        <KpiCard label="반 석차" value={summary?.overallClassRank != null ? `${summary.overallClassRank}위` : '-'} color="#2471b8" />
        <KpiCard
          label="출석률"
          value={summary?.attendanceRate != null ? `${(summary.attendanceRate).toFixed(1)}%` : '-'}
          sub={summary ? `결석 ${summary.absentCount ?? 0} · 지각 ${summary.lateCount ?? 0} · 조퇴 ${summary.earlyLeaveCount ?? 0}` : undefined}
          color="#2ecc71"
        />
        <KpiCard label="위험 여부" value={summary?.riskFlag ? '관심 필요' : '양호'} color={summary?.riskFlag ? '#e74c3c' : '#2ecc71'} />
      </div>

      <div className="analytics-chart-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
...
function ClassAnalyticsView({ isAdmin }: { isAdmin: boolean }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const mapped = courseList.map((c: any) => ({ id: c.id, name: c.courseName }));
      setCourses(mapped);
  ...
  useEffect(() => { if (classGroupId) load(); }, [load, classGroupId]);
  ...
  const selectStyle: React.CSSProperties = {
    ...
    fontSize: '13px', fontFamily: "'Noto Sans KR', sans-serif", outline: 'none', background: '#fff', cursor: 'pointer',
  };
  ...
  return (
    <div>
      <style>{`
        @media (max-width: 768px) {
          .class-analytics-charts { grid-template-columns: 1fr !important; }
          .class-analytics-controls { flex-direction: column !important; align-items: stretch !important; }
          .class-analytics-controls > div,
          .class-analytics-controls > button { width: 100% !important; }
          .student-analysis-controls { flex-direction: column !important; align-items: stretch !important; }
          .student-analysis-controls > select { width: 100% !important; }
          .student-info-row { flex-direction: column !important; }
          .student-radar-grid { grid-template-columns: 1fr !important; }
          .student-detail-table-wrap { overflow-x: auto; }
        }
      `}</style>

      <Card>
        <div className="class-analytics-controls" style={{ display: 'flex', alignItems: 'flex-end', gap: '16px', flexWrap: 'wrap' }}>
          <div>
            <label style={{ display: 'block', fontSize: '12px', color: '#64748b', fontWeight: 600, marginBottom: '6px' }}>담당 반</label>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '12px', color: '#64748b', fontWeight: 600, marginBottom: '6px' }}>과목</label>
            {courseOptions.length > 0 ? (
              <select value={courseId} onChange={(e) => setCourseId(Number(e.target.value))} style={selectStyle}>
                {courseOptions.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            ) : (
              <div style={{ padding: '8px 12px', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '13px', color: '#94a3b8' }}>개설 과목 없음</div>
            )}
          </div>
          <Button variant="secondary" onClick={load}>조회</Button>
          {isAdmin && <Button variant="primary" onClick={handleEtl}>분석 데이터 갱신(ETL 실행)</Button>}
        </div>
      ...
      <div className="class-analytics-charts" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        <Card title="과목별 반 평균">
          {validStats.length > 0 ? (
            <ClassAverageBarChart stats={validStats} courseMap={Object.fromEntries(courses.map((c) => [c.id, c.name]))} />
          ) : <EmptyState />}
        </Card>
        <Card title="점수 분포">
      ...
      <Card title="학생 개인 분석">
        <div className="student-analysis-controls" style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        {selectedStudentId > 0 && studentCourses.length > 0 ? (() => {
          ...
          return (
            <div>
              <div className="student-info-row" style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px', padding: '12px 16px', background: '#f8fafc', borderRadius: '10px' }}>
                ...
                <div style={{ marginLeft: 'auto', textAlign: 'right', display: 'flex', gap: '20px', alignItems: 'center', flexWrap: 'wrap' }}>
                  {avgGrade != null && (
              ...
              <div className="student-radar-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '28px', alignItems: 'stretch' }}>
                <div>
