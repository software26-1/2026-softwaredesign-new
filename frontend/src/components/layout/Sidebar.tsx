...
const ICONS = {
  ...
  person:   (p: IconProps) => <Svg {...p}><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></Svg>,
};
...
const teacherNav: NavItem[] = [
  ...
  { path: '/mypage',          label: '마이페이지',  icon: 'person'   },
];
...
export function Sidebar({ role, classGroupId, onChatbotToggle, isCollapsed = false, onToggleCollapse, onNavigate, isDrawer = false }: SidebarProps) {
  ...
  if (!isDrawer) {
    navItems = navItems.filter(item => item.path !== '/mypage');
  }
              ...
              {!isCollapsed && item.subItems && (
                <div style={{ paddingLeft: '8px', paddingBottom: '6px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px' }}>
                  {item.subItems.map(sub => (
                    <button
                      key={sub.tab}
                      onClick={() => { navigate(sub.tab ? `${item.path}?tab=${sub.tab}` : item.path); onNavigate?.(); }}
